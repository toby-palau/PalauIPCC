...
const Page = async ({
	...
	params: { quizId: QuizIdType };
	searchParams: { randomSeed: string };
}) => {
	...
	const chapterStats = chapters.map((c) => {
		const filteredQuestions = questions.filter((q) =>
			c.questionIds.includes(q.questionId)
		);
		const stats = filteredQuestions.reduce(
			(acc, q) => ({
				started: Math.max(acc.started, q.response_count),
				finished:
					filteredQuestions.length < c.questionIds.length
						? 0
						: acc.finished > 0
						? Math.min(acc.finished, q.response_count)
						: q.response_count,
		...
		<div className="min-h-screen w-screen md:p-10 p-2 bg-white-dark">
			<div className="flex flex-row justify-end items-center">
				<a
					href={`/${
						params.quizId
					}/hidden-dashboard?randomSeed=${Math.floor(
						Math.random() * 1000
					)}`}
					className="m-2 p-2 text-blue-dark text-1xl border border-blue-dark rounded"
				>
					Refresh
				</a>
			</div>
				...
				<div className="md:col-span-2 col-span-3 bg-white p-2 h-96 overflow-scroll text-black">
					<div className="h-full w-full flex flex-col justify-center items-center">
						<h1 className="text-center text-lg">
							Response count last 7 days
						</h1>
						<BarChart
							data={responseCountsByDate.map((c) => ({
								...c,
								date: c.date.toLocaleDateString("en-US", {
									day: "numeric",
									month: "short",
								}),
							}))}
