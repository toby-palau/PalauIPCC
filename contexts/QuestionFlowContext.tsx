...
import {
	createNewResponse,
	resetExistingResponse,
} from "@root/services/DatabaseService";
...
export const QuestionFlowProvider = ({
	children,
	quizId,
	initialSession,
	nextChapterId,
	...
	const [currentPage, setCurrentPage] =
		useState<ChapterType["pages"][number]>();
	const [triggerTimeoutToNextQuestion, setTriggerTimeoutToNextQuestion] =
		useState<boolean>(false);
	const [disableQuestionAnswers, setDisableQuestionAnswers] =
		useState<boolean>(false);
	const [timeoutId, setTimeoutId] = useState<NodeJS.Timeout>();

	useEffect(() => {
		const lastQuestionIndex = initialSession.pages.findLastIndex(
			(p) => p.pageType === PageTypes.question && !p.question.skippable
		);
		const lastCompletedIndex = initialSession.pages.findLastIndex(
			(p) => p.pageType === PageTypes.question && p.completed
		);
		...
		if (lastCompletedIndex < 0) setCurrentIndex(0);
			...
			if (successIndex < 0) setCurrentIndex(lastCompletedIndex);
			else setCurrentIndex(successIndex);
	}, []);
	...
	useEffect(() => {
		...
		const handleKeyPressEvent = (e: KeyboardEvent) => {
			if (e.key === "ArrowRight")
				navigate(currentIndex, currentIndex, "forward", false);
			if (e.key === "ArrowLeft")
				navigate(currentIndex, currentIndex, "backward", false);
	...
	const submitResponse = async (
		...
		const newPage = await updateQuestionPage(page, {
			userAnswer,
			questionType: page.question.questionType,
		});
		if (newPage.answeredCorrectly) triggerConfetti();

		if (
			userSession.pages.filter(
				(p) => p.pageType === PageTypes.question && p.completed
			).length <= 0
		)
			track("Submit First Question In Chapter", {
				chapterId: userSession.cid,
				chapterTitle: userSession.chapterTitle,
			});
		if (
			userSession.pages.findLastIndex(
				(p) =>
					p.pageType === PageTypes.question && !p.question.skippable
			) === currentIndex
		)
			track("Submit Last Question In Chapter", {
				chapterId: userSession.cid,
				chapterTitle: userSession.chapterTitle,
			});
		...
		const newSession = {
			...userSession,
			pages: userSession.pages.map((p) =>
				p.pid === pageId ? newPage : p
			),
		};
	...
	const skipQuestion = (pageId: string) => {
		...
		setUserSession({
			...userSession,
			pages: userSession.pages.map((p) =>
				p.pid === pageId ? { ...p, completed: true } : p
			),
		});
		navigate(currentIndex, currentIndex, "forward", false);
	};
	...
	const calculateScore = () => {
		...
		const correct = userSession.pages.filter(
			(p) => p.pageType === PageTypes.question && p.answeredCorrectly
		).length;
		const total = userSession.pages.filter(
			(p) =>
				p.pageType === PageTypes.question &&
				p.completed &&
				p.question.correctAnswer !== undefined
		).length;
	...
	const navigate = (
		...
		if (direction === "forward") {
			if (fromIndex >= userSession.pages.length - 1)
				return router.push(
					nextChapterId
						? `/${quizId}/chapters/${nextChapterId}`
						: `/${quizId}`
				);
			if (shiftDown) return setCurrentIndex(fromIndex + 1);
			if (
				!skippedQuestion &&
				currentQuestion.pageType === PageTypes.question &&
				!currentQuestion.completed
			)
				return setCurrentIndex(fallbackIndex);
			toIndex++;
		}
	...
	const checkDisplayLogic = (displayLogic: DisplayLogicType) => {
		...
		if (displayLogic.type === DisplayLogicTypes.answeredCorrectly) {
			...
			const logicQuestion = userSession.pages.find(
				(p) => p.pid === logicQuestionId
			);
			if (
				!logicQuestion ||
				logicQuestion?.pageType !== PageTypes.question
			)
				return false;
			if (
				logicQuestion.completed &&
				logicQuestion.answeredCorrectly === displayLogic.correct
			)
				return true;
			else return false;
		}
		if (displayLogic.type === DisplayLogicTypes.seenBefore) {
			const ts = localStorage.getItem(
				displayLogic.localStorageIndentifier
			);
			if (ts) return false;
