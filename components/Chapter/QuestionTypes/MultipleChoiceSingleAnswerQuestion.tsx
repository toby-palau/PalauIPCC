...
export const MultipleChoiceSingleAnswerQuestion = (props: {question: MCSAQuestionType, disabled: boolean, submitResponse: (s: Array<string>) => void}) => {
        ...
        <div className="max-w-xl m-auto flex-col items-center justify-between py-6 px-6 border-2 border-white rounded bg-black">
            ...
            <div className="flex flex-col items-center justify-between">
                {props.question.options.map(o => {
                    let className = `${dmsans.className} w-full cursor-pointer sm:text-xl text-md text-center text-black rounded-md my-0.5 p-3 ${!props.disabled && "active:scale-95"} transition-all duration-100`;
